...
void alterar_produto() {
    ...
    switch (opcao) {
        ...
        case 3:
            ...
            break;
        case 4: {
            float quantidade_anterior = p->quantidade;
            printf("Digite a nova quantidade (em kg): ");
            p->quantidade = entrada_float_valida();
            float diferenca = p->quantidade - quantidade_anterior;
             if (diferenca > 0) {
        // Adicionou mais quantidade
            float valor_saida = diferenca * p->preco_fornecedor;
          
            salvar_transacao("Aumento de estoque", valor_saida, "Saída");
            } else if (diferenca < 0) {
        // Removeu quantidade
            float valor_prejuizo = (-diferenca) * p->preco_fornecedor;
            
            salvar_transacao("Diminuição de estoque", valor_prejuizo, "Prejuízo");
    }
            break;
			}
        case 5:
